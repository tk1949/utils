...
public class Util {
    ...
    public static long digest(final byte[] data) {
        ...
        for (byte aData : data) {
            final int lookupidx = ((byte) checksum ^ aData) & 0xff;
    ...
    public static boolean byteSort(byte[] _1, byte[] _2) {
        if (_1.length == _2.length) {
            for (int i = 0, len = _1.length; i < len; i++) {
                if (_1[i] != _2[i]) {
                    return _1[i] < _2[i];
                }
            }
    ...
    public static long FNV64(final String k) {
        long rv = FNV_64_INIT;
        int len = k.length();
